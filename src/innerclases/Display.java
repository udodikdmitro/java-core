    ...
    private class Pixel {
        ...
        private Pixel(int x, int y, Color color) {
            if (0 <= x && x <= DISPLAY_WIDTH && 0 <= y && y <= DISPLAY_HEIGHT) {
                ...
                throw new IllegalArgumentException("Coordinates x and why should be " +
                        "between 0-"+DISPLAY_WIDTH+" and 0-"+DISPLAY_HEIGHT);
            }
        }
    }

    public enum Color {
        BLACK, RED, GREEN, CYAN, MAGENTA, BLUE, YELLOW
    }
}
